...
class ProviderManagerImpl {
  ...
  registerRegistry(
    ...
    logger.info(`Registered registry for category: ${category}`);
  }
  ...
  async getSandboxProvider(): Promise<IProvider | undefined> {
    ...
    if (!registry) {
      logger.warn('No sandbox registry registered');
      return undefined;
    }
  ...
  async getAgentProvider(): Promise<IProvider | undefined> {
    ...
    if (!registry) {
      logger.warn('No agent registry registered');
      return undefined;
    }
  ...
  async getProvider(category: string): Promise<IProvider | undefined> {
    ...
    if (!registry) {
      logger.warn(`No registry for category: ${category}`);
      return undefined;
    }
  ...
  async switchSandboxProvider(
    ...
    logger.info(`Switched sandbox provider to: ${type}`);
  }
  ...
  async switchAgentProvider(
    ...
    logger.info(`Switched agent provider to: ${type}`);
  }
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      logger.warn('Could not load sandbox registry:', error);
    }
    ...
    try {
      ...
      logger.warn('Could not load agent registry:', error);
    }
    ...
    logger.info('Initialized with config:', {
      sandbox: this.config.sandbox
        ? {
            category: this.config.sandbox.category,
            type: this.config.sandbox.type,
          }
        : undefined,
      agent: this.config.agent
        ? { category: this.config.agent.category, type: this.config.agent.type }
        : undefined,
    });
  }
  ...
  async shutdown(): Promise<void> {
    ...
    for (const [category, provider] of this.activeProviders) {
      try {
        ...
        logger.info(`Stopped ${category} provider`);
      } catch (error) {
        logger.warn(`Error stopping ${category} provider:`, error);
      }
    }
    ...
    for (const [category, registry] of this.registries) {
      try {
        ...
        logger.info(`Stopped all ${category} providers`);
      } catch (error) {
        logger.warn(`Error stopping ${category} registry:`, error);
      }
    }
