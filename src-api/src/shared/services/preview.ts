...
export interface PreviewConfig {
  taskId: string;
  workDir: string; // Host path: app data directory sessions/{taskId}
...
type PreviewCommand = 'node' | 'npm' | 'npx';
...
export class PreviewManager {
  ...
  async startPreview(config: PreviewConfig): Promise<PreviewStatus> {
    ...
    this.startViteServer(instance, workDir).catch((error) => {
      logger.error(`Failed to start preview for ${taskId}:`, error);
  ...
  private async startViteServer(
    ...
    try {
      ...
      logger.info(
        `Starting Vite server for ${instance.taskId} on port ${instance.port}`,
      );

      await this.startViteProcess(instance, workDir);
    } catch (error) {
      logger.error(`Error starting Vite server:`, error);
      instance.status = 'error';
      instance.error = error instanceof Error ? error.message : String(error);
      throw error;
    }
  ...
  private async startViteProcess(
    ...
    try {
      ...
      logger.info('Vite already installed, skipping npm install');
    } catch {
      needsInstall = true;
    }

    if (needsInstall) {
      logger.info('Vite not found, installing dependencies...');
      ...
      await new Promise<void>((resolve, reject) => {
        const npmInstall = spawn(resolvePreviewCommand('npm'), ['install'], {
          cwd: workDir,
          shell: false,
          stdio: 'pipe',
        });
        ...
        npmInstall.on('close', (code) => {
          ...
          if (code === 0) {
            logger.info(`npm install completed in ${elapsed}s`);
            resolve();
          } else {
            reject(
    ...
    // Start Vite
    logger.info(`Starting Vite dev server on port ${instance.port}...`);
    ...
    if (fsSync.existsSync(viteCliPath)) {
      ...
      viteCmd = resolvePreviewCommand('node');
      viteArgs = [viteCliPath];
      logger.info(`Running: node ${viteCliPath}`);
    } else {
      // Fallback to npx
      viteCmd = resolvePreviewCommand('npx');
      viteArgs = ['vite'];
      logger.info('Running: npx vite');
    }
    ...
    viteProcess.on('error', (error) => {
      logger.error(`Vite process error:`, error);
      instance.status = 'error';
    ...
    if (isReady) {
      ...
      logger.info(`Vite server running at http://localhost:${instance.port}`);
    } else {
      instance.status = 'error';
  ...
  private async waitForServerReady(
    ...
    logger.info(
      `Waiting for server on port ${port} (timeout: ${timeout / 1000}s)...`,
    );
    ...
    while (Date.now() - startTime < timeout) {
      ...
      try {
        ...
        if (response.ok || response.status === 404) {
          ...
          logger.info(
            `Server ready on port ${port} after ${attempts} attempts (${((Date.now() - startTime) / 1000).toFixed(1)}s)`,
          );
          return true;
        }
      } catch {
        ...
        if (attempts % 10 === 0) {
          ...
          logger.info(
            `Still waiting for server... (${elapsed}s elapsed, ${attempts} attempts)`,
          );
        }
      }
    ...
    logger.warn(
      `Server failed to start within ${timeout / 1000}s (${attempts} attempts)`,
    );
    return false;
  }
  ...
  private async ensureProjectFiles(
    ...
    for (const configPath of [
      ...
      try {
        ...
        logger.info(`Removed conflicting config: ${configPath}`);
      } catch {
    ...
    logger.info(`Writing vite.config.js with port ${port}`);
...
<body>
  <p>Redirecting to <a href="./${htmlFile}">${htmlFile}</a>...</p>
        ...
        logger.info(`Created index.html redirecting to ${htmlFile}`);
      } else {
        logger.warn('No HTML file found in workDir');
      }
    }
  }
  ...
  async stopPreview(taskId: string): Promise<PreviewStatus> {
    ...
    logger.info(`Stopping preview for ${taskId}`);
  ...
  private startHealthCheck(instance: PreviewInstance): void {
    ...
    instance.healthCheckInterval = setInterval(async () => {
      ...
      try {
        ...
        logger.warn(`Health check failed for ${instance.taskId}:`, error);
        instance.status = 'error';
  ...
  private resetIdleTimeout(instance: PreviewInstance): void {
    ...
    instance.idleTimeout = setTimeout(() => {
      logger.info(`Idle timeout reached for ${instance.taskId}`);
