 ...
 *
 * Ported verbatim from Open Design's `apps/daemon/src/design-system-showcase.ts`
 * so our "Showcase" view matches theirs exactly (the uniform "The system that
 * makes X feel like X" template). This is distinct from each system's bundled
 * `components.html`, which is the bespoke "reference components" fixture
 * surfaced separately as the modal's "Reference" tab.
 *
...
export function renderDesignSystemShowcase(id: string, raw: string): string {
  ...
  const subtitle =
    extractSubtitle(raw) ||
    'A design system rendered as a real product surface.';
...
function inlineLineChart(): string {
  ...
  const points = data
    .map((v, i) => `${padX + i * stepX},${norm(v).toFixed(1)}`)
    .join(' ');
...
export function extractColors(raw: string): ColorToken[] {
    ...
    if (seen.has(key)) {
      ...
      if (cleanRole) {
        const existing = colors.find(
          (c) =>
            c.name.toLowerCase() === cleanName.toLowerCase() && c.value === v,
        );
        if (
          existing &&
          (!existing.role || cleanRole.length > existing.role.length)
        ) {
  ...
  for (const rawLine of raw.split(/\r?\n/)) {
    ...
    // and "Text" labels.
    const spec =
      /^[\s>*-]*\*{0,2}([A-Za-z][^:*\n]{1,40}?)\*{0,2}\s*[:：]\s*\*{0,2}\s*`?(#[0-9a-fA-F]{3,8})/.exec(
        line,
      );
    if (spec) {
...
function extractFonts(raw: string): FontHints {
  ...
  while ((m = re.exec(raw)) !== null) {
    ...
    const value = (m[2] ?? '')
      .trim()
      .replace(/[*_`]+$/g, '')
      // Font values are injected raw into a `<style>` block as
      // `--display: <value>`. Strip characters that could break out of the CSS
      // value or the style element (`< > { } ; \`) so a hand-crafted DESIGN.md
      // can't smuggle markup/extra rules into the generated showcase. Legit
      // font-family stacks never contain these.
      .replace(/[<>{};\\]/g, '')
      .trim();
    if (!/[a-zA-Z]/.test(value)) continue;
    if (value.startsWith('#')) continue;
    if (/display|heading|h1|title/.test(label) && !out.display)
      out.display = value;
    else if (/body|text|paragraph|copy/.test(label) && !out.body)
      out.body = value;
...
function pickColor(
  ...
  for (const hint of hints) {
    const byRole = colors.find(
      (c) => isAllowed(c) && matchesHint(c.role, hint),
    );
    if (byRole) return byRole.value;
    const byName = colors.find(
      (c) => isAllowed(c) && matchesHint(c.name, hint),
    );
...
function mixSurface(bg: string): string {
  ...
  const fix = (v: number) =>
    Math.max(0, Math.min(255, v + adjust))
      .toString(16)
      .padStart(2, '0');
