...
// Initialize sandbox module on first request (promise-guarded against concurrent calls)
let initPromise: Promise<void> | null = null;
async function ensureInitialized() {
  if (!initPromise) {
    initPromise = initSandbox();
  }
  await initPromise;
}
...
async function getProviderWithFallback(
  ...
  logger.debug(
    `getProviderWithFallback called with: ${preferredProvider || 'auto'}`,
  );

  if (!preferredProvider) {
    logger.debug('No preferred provider, using getBestProvider()');
    return { provider: await getBestProvider(), usedFallback: false };
  }

  try {
    logger.debug(`Attempting to get provider: ${preferredProvider}`);
    const provider = await getSandboxProvider(preferredProvider);
    logger.debug('Got provider instance, checking availability...');

    // Check if provider is available
    const isAvailable = await provider.isAvailable();
    logger.debug(`Provider ${preferredProvider} isAvailable: ${isAvailable}`);

    if (isAvailable) {
      logger.debug(`Using provider: ${preferredProvider}`);
      return { provider, usedFallback: false };
    }
    logger.warn(
      `Provider ${preferredProvider} not available, falling back to native`,
    );
  } catch (error) {
    logger.error(`Failed to get provider ${preferredProvider}:`, error);
  }

  // Fallback to native
  logger.info('Using native fallback');
...
sandbox.get('/debug/codex-paths', async (c) => {
  ...
  return c.json({
    ...
    cwd: getSetting('workDir') ?? process.cwd(),
...
sandbox.post('/exec', async (c) => {
  try {
    ...
    if (usedFallback) {
      logger.info('Using native fallback for exec');
    }
...
sandbox.post('/run/file', async (c) => {
  try {
    ...
    logger.debug(`Running file: ${filePath}`);
    ...
    if (!preferredProvider && needsNetwork) {
      logger.info(
        'Network packages detected, using native provider for proxy support',
      );
    ...
    if (usedFallback) {
      logger.info('Using native fallback for run/file');
    }
    ...
    logger.error('File run error:', error);
    return c.json(
      {
        success: false,
        error: error instanceof Error ? error.message : String(error),
...
sandbox.post('/run/node', async (c) => {
  try {
    ...
    const scriptPath = `${workDir}/temp_script_${crypto.randomUUID()}.js`;
    ...
    logger.error('Node run error:', error);
    return c.json(
      {
        success: false,
        error: error instanceof Error ? error.message : String(error),
