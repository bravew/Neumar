...
export class DeepAgentsAdapter extends BaseAgent {
  ...
  private async initializeAgent(_tools: unknown[] = []) {
    ...
    */
    ...
    logger.warn('Package not installed. Please install @langchain/deepagents');
    return null;
  }
  ...
  async *run(
    ...
    logger.info(`${session.id} Direct execution started`);
  ...
  async *execute(options: ExecuteOptions): AsyncGenerator<AgentMessage> {
    ...
    logger.info(`${session.id} Using plan: ${plan.id} (${plan.goal})`);
