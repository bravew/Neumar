...
class SandboxRegistry implements SandboxProviderRegistry {
    ...
    if (typeof typeOrPlugin === 'string') {
      ...
      const legacyPlugin: SandboxPlugin = {
        metadata: {
          ...
          // Legacy registrations have unknown enforcement; assume worst case
          // so they cannot be marketplace-eligible without an explicit plugin.
          enforcement: 'none',
  ...
  private registerPlugin(plugin: SandboxPlugin): void {
    ...
    if (this.plugins.has(type)) {
      logger.warn(`Overwriting existing provider: ${type}`);
    }
    this.plugins.set(type, plugin);
    logger.info(`Registered provider: ${type} (${plugin.metadata.name})`);
  }
  ...
  async getInstance(
    ...
    if (instanceData && instanceData.state === 'error') {
      logger.info(`Recreating provider ${type} after error`);
  ...
  async stopAll(): Promise<void> {
      ...
      if (instance.state === 'ready' || instance.state === 'error') {
        stopPromises.push(
          instance.provider.stop().catch((error) => {
            logger.warn(`Error stopping provider ${type}:`, error);
          }),
        );
      }
    }
    ...
    logger.info('All provider instances stopped');
  }
