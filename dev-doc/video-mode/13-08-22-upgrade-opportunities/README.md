...
Status: core implementation complete through Phase D; selected Phase E items
complete; Phase F gated on real Studio usage
...
These are the six findings recorded at the `5dafc51` baseline, ranked by
value-to-effort. Their current implementation state is noted inline; the exact
commit and verification ledger is in
[`05-implementation-plan.md`](05-implementation-plan.md).

1. **`@remotion/effects` was not installed.** Remotion ships 72 GPU shader
   ...
   capability-per-line-of-code win available. → **P0 — implemented in A1/A2**

2. **The `html` engine recorded the browser in real time.**
   ...
   builds. → **P0 — HyperFrames adapter implemented; `html` retained as an
   explicit fallback**

3. **`hyperframes preview --context --json` was an unused agent bridge.** It
   returns the Studio-selected element's `data-hf-id`, selector,
   ...
   (`player.js`) for in-app preview. → **P1 — implemented in C1**

4. **Beat-driven editing had no representation in Neuma.** `hyperframes beats`
...
6. **Neuma's bundled HyperFrames skill was two minors stale.**
   ...
   → **P1 — upstream router, Neuma reference, and CI drift guard implemented**
...
- At the review baseline, `@remotion/effects`, `@remotion/media`,
  `@remotion/animation-utils`, `@remotion/webcodecs`, and
  `@remotion/mac-cursors` published 4.0.515 and were absent from Neuma. A1/A2
  subsequently added the first two exact pins.
- The review enumerated 105 Neuma `video_*` tools and 276 OpenReel tools. The
  implemented branch now registers 111 Video tools, all covered by permission
  and cost metadata tests.
