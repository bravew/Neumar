...
export function useVitePreview(taskId: string | null): UseVitePreviewReturn {
  ...
  // Reset state when taskId changes — fetch status with proper cleanup
  useEffect(() => {
    if (!taskId) {
      setPreviewUrl(null);
      setStatus('idle');
    ...
    (async () => {
      try {
        const response = await fetch(
          `${AGENT_SERVER_URL}/preview/status/${taskId}`,
          { signal: ac.signal },
        );
    ...
    return () => ac.abort();
  }, [taskId, updateStateFromResponse]);
    ...
    async (workDir: string) => {
      ...
      try {
        if (import.meta.env.DEV) {
          ...
          console.warn('[useVitePreview] workDir:', workDir);
        }
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.error ||
              `Failed to start preview: ${response.statusText}`,
          );
        }

        const data: PreviewApiResponse = await response.json();
        if (import.meta.env.DEV) {
          console.warn('[useVitePreview] Start response:', data);
        }
  ...
  const stopPreview = useCallback(async () => {
    ...
    try {
      if (import.meta.env.DEV) {
        console.warn(
          '[useVitePreview] Stopping preview for:',
          taskIdRef.current,
        );
      }
