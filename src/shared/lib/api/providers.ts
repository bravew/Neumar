...
export interface SandboxProviderMetadata extends ProviderMetadata {
  ...
  // Phase 7 enforcement metadata. Optional in the frontend type so older API
  // responses keep deserializing; UI should treat missing as 'none'/false.
...
export async function switchSandboxProvider(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      error.error ||
        `Failed to switch sandbox provider: ${response.statusText}`,
    );
  }
  return response.json();
}
