...
/** Map a file extension (lowercase, no dot) to an {@link ArtifactType}.
 *  Single source of truth — imported by TaskDetail, RightSidebar, etc. */
...
export function inlineAssets(html: string, allArtifacts: Artifact[]): string {
  ...
  result = result.replace(cssRegex, (match, filename) => {
    ...
    if (cssArtifact?.content) {
      console.warn('[ArtifactPreview] Inlining CSS:', filename);
  ...
  result = result.replace(jsRegex, (match, filename) => {
    ...
    if (jsArtifact?.content) {
      console.warn('[ArtifactPreview] Inlining JS:', filename);
