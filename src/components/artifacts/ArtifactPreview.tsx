...
import { DeclarativeArtifact } from './declarative/DeclarativeArtifact';
...
export function ArtifactPreview({
  ...
  // Lazy-load full file content from disk when artifact has a path but only a
  // truncated preview in `content`. This avoids storing large files in the DB.
  useEffect(() => {
    if (!artifact?.path) {
      setFullContent(null);
      return;
  ...
  // Uses AbortController to cancel the fetch on unmount
  useEffect(() => {
  ...
  // Use full file content from disk when available (overrides the truncated DB preview).
  // Downstream code reads `displayContent` for rendering.
  const displayContent = fullContent ?? artifact?.content;
  ...
  }, [artifact?.id, artifact?.type]); // Only respond to id/type changes, not full artifact object

  // Handle copy to clipboard — prefer full file content when available
  const handleCopy = async () => {
    const textToCopy = displayContent ?? artifact?.content;
  ...
  // Reveal the file's containing folder in Finder / Explorer.
  // Backed by the same /files/open endpoint used elsewhere — passing a
  // directory path makes the OS open it in the file manager. macOS Finder,
  // Windows Explorer, and Linux xdg-open all do the right thing.
  const handleOpenContainingFolder = async () => {
    if (!artifact?.path) return;
    const idx = artifact.path.lastIndexOf('/');
    if (idx <= 0) return;
    const folder = artifact.path.slice(0, idx);
    try {
      ...
      if (!result.success && import.meta.env.DEV) {
        console.warn(
          '[ArtifactPreview] Failed to open containing folder:',
          result.error,
        );
      }
    } catch (err) {
  ...
  const handleOpenInEditor = async () => {
    ...
    try {
      console.warn('[ArtifactPreview] Opening in editor:', artifact.path);
      ...
      const result = await response.json();
      if (result.success) {
        console.warn('[ArtifactPreview] Opened in', result.editor);
      } else {
        console.error(
  ...
  // Generate iframe blob URL for HTML static preview.
  // Uses a ref to track the current blob URL and avoid leaking when
  // allArtifacts changes (new array reference on every parent render).
  const iframeSrcRef = useRef<string | null>(null);
  const [iframeSrc, setIframeSrc] = useState<string | null>(null);
  const prevHtmlKeyRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    if (
      ...
      // Revoke any existing blob URL when preview is hidden
      if (iframeSrcRef.current) {
        URL.revokeObjectURL(iframeSrcRef.current);
        iframeSrcRef.current = null;
      }
      setIframeSrc(null);
      prevHtmlKeyRef.current = null;
      return;
    }

    // Only recreate if the artifact content, classification, or sibling assets changed.
    // Include isOutput so re-classification that replaces the array reference
    // (without changing IDs or content) still triggers a blob URL refresh.
    const htmlKey =
      artifact.id +
      ':' +
      String(artifact.isOutput ?? false) +
      ':' +
      artifact.content.length +
      ':' +
      allArtifacts.map((a) => a.id).join(',');
    if (htmlKey === prevHtmlKeyRef.current) return;
    prevHtmlKeyRef.current = htmlKey;
  ...
  const hasPreview = useMemo(() => {
    ...
    switch (artifact.type) {
      ...
      case 'markdown':
        return !!artifact.content || !!artifact.path;
      case 'mermaid':
        return !!displayContent || !!artifact.path;
      case 'declarative':
        return !!displayContent || !!artifact.path;
      case 'csv':
        return !!csvData;
  ...
  }, [artifact, csvData, slides, displayContent]);
  ...
  return (
    ...
    >
          ...
          <div className="flex shrink-0 items-center gap-1">
            ...
            {artifact.path && (
              <Tooltip>
                ...
                <TooltipContent side="bottom">
                  <p>{t.preview.openContainingFolder}</p>
                </TooltipContent>
              </Tooltip>
            )}
      ...
      {/* Content — min-h-0 allows this flex child to shrink below content size
           so the inner scroll container gets a fixed height and h-scrollbar
           stays at the viewport bottom instead of the content bottom. */}
      <div className="min-h-0 flex-1 overflow-hidden">
        {viewMode === 'preview' ? (
          previewMode === 'live' && canUseLivePreview ? (
            ...
            <PreviewContent
              artifact={
                displayContent && displayContent !== artifact.content
                  ? { ...artifact, content: displayContent }
                  : artifact
              }
              iframeSrc={iframeSrc}
              iframeRef={iframeRef}
...
function PreviewContent({
  ...
  // Mermaid Preview — render with the dedicated MermaidView (mermaid@11
  // direct, parse-then-render, DOMPurify, LRU-cached). See
  // `src/components/mermaid/MermaidView.tsx`.
  if (artifact.type === 'mermaid' && artifact.content) {
    return (
      <div className="bg-background h-full overflow-auto">
        <MermaidView source={artifact.content} className="p-4" />
      </div>
    );
  }

  if (artifact.type === 'declarative' && artifact.content) {
    return <DeclarativeArtifact content={artifact.content} />;
  }
    ...
    return (
      <div className="bg-background h-full overflow-auto">
        <div className="max-w-none p-6">
          ...
          {/* Markdown Content */}
          <MarkdownPreviewBody content={markdownContent} />
        </div>
      </div>
    );
  }
...
/**
 * Theme-aware markdown body shared between the markdown preview branch and
 * the lightbox. Pulls plugins via `useStreamdownPlugins` so fenced
 * ```mermaid blocks pick up the right theme + edge-contrast variables.
 */
function MarkdownPreviewBody({ content }: { content: string }) {
  const plugins = useStreamdownPlugins();
  // Inject paired `color:` per `style X fill:` line so user-supplied
  // fills get readable text contrast (YIQ luminance threshold).
  const processed = useMemo(
    () => preprocessMermaidInMarkdown(content),
    [content],
  );
  return (
    <div
      className={cn(
        ...
        'prose-h1:text-xl prose-h1:font-semibold prose-h2:text-lg prose-h2:font-semibold',
        ...
        'dark:prose-th:border-border dark:prose-td:border-border [&_thead]:bg-muted [&_th]:border-border [&_td]:border-border [&_tbody_tr:nth-child(even)]:bg-muted/30 max-w-none [&_table]:w-full [&_table]:border-collapse [&_table]:text-sm [&_td]:border [&_td]:px-3 [&_td]:py-2 [&_th]:border [&_th]:px-3 [&_th]:py-2 [&_th]:text-left [&_th]:font-medium [&_thead]:sticky [&_thead]:top-0',
        // Mermaid edge stroke (no config key in v11; CSS canonical).
        '[&_svg.flowchart_.flowchart-link]:!stroke-[1.75px]',
        '[&_svg.flowchart_.edgePath_path]:!stroke-[1.75px]',
        // Prose `p { color: ... }` overrides our per-node color injection
        // because mermaid wraps labels in <p>. Force inherit from the
        // span (which carries the YIQ-contrast color).
