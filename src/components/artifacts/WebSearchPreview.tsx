...
export function parseSearchResults(content: string): SearchGroup[] {
      ...
      if (jsonStr) {
        try {
          ...
          if (Array.isArray(links)) {
            ...
            console.warn('[WebSearchPreview] Parsed', links.length, 'links');
          }
        } catch (e) {
          console.error('[WebSearchPreview] JSON parse error:', e);
        }
      }
    }
  ...
  console.warn(
    '[WebSearchPreview] Links sections found:',
...
export function WebSearchPreview({ artifact }: { artifact: Artifact }) {
  ...
  console.warn('[WebSearchPreview] Content length:', content.length);
  console.warn('[WebSearchPreview] Content preview:', content.slice(0, 200));
  console.warn('[WebSearchPreview] Parsed groups:', groups.length, groups);
