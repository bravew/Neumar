import type { VideoClipFilters } from '@/shared/types/video';

const NEUTRAL_MULTIPLIER = 1;
const NEUTRAL_ZERO = 0;
const EPSILON = 0.001;

export function normalizeVideoClipFilters(
  filters: VideoClipFilters | undefined,
): VideoClipFilters | undefined {
  if (!filters) return undefined;
  const normalized: VideoClipFilters = {};
...
export function buildVideoClipCssFilter(
  filters: VideoClipFilters | undefined,
): string | undefined {
  const normalized = normalizeVideoClipFilters(filters);
