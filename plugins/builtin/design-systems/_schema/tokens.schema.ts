 ...
 *
 * Each token in the Neuma DesignMode ecosystem belongs to exactly one of
 ...
 */
export const BRAND_EXTENSION_PREFIXES: readonly string[] = ['--tag-bg-'];
...
export function getRequiredA1Names(): readonly string[] {
  return TOKEN_SCHEMA.filter(
    (t) => t.layer === 'A1-identity' || t.layer === 'A1-structure',
  ).map((t) => t.name);
}
...
export function isAllowedExtension(brand: string, name: string): boolean {
  if (BRAND_EXTENSION_PREFIXES.some((prefix) => name.startsWith(prefix)))
    return true;
