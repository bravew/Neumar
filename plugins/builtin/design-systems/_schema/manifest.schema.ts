...
export const DESIGN_SYSTEM_PROJECT_SCHEMA_VERSION =
  'od-design-system-project/v1' as const;
...
export type DesignSystemProjectImportMode =
  | 'normalized'
  | 'hybrid'
  | 'verbatim';
...
const ALLOWED_SOURCE_KEYS: Record<
  DesignSystemProjectSource['type'],
  ReadonlySet<string>
> = {
...
const ALLOWED_FILES_KEYS = new Set([
  'design',
  'tokens',
  'designTokens',
  'tailwind',
...
const ALLOWED_SOURCE_FILES_KEYS = new Set([
  'scanned',
  'evidence',
  'tokens',
  'report',
...
export function validateDesignSystemProjectManifest(
  ...
  if (value.description !== undefined)
    expectNonEmptyString(errors, '$.description', value.description);

  validateSource(errors, value.source);
  validateFiles(errors, value.files);

  if (value.assetsDir !== undefined)
    expectLiteral(errors, '$.assetsDir', value.assetsDir, 'assets');
  if (value.previewDir !== undefined)
    expectLiteral(errors, '$.previewDir', value.previewDir, 'preview');
  if (value.usage !== undefined)
    expectSafeRelativePath(errors, '$.usage', value.usage);
  if (value.componentsManifest !== undefined) {
    expectSafeRelativePath(
      errors,
      '$.componentsManifest',
      value.componentsManifest,
    );
  }
  if (value.importMode !== undefined)
    validateImportMode(errors, value.importMode);
  ...
  if (value.sourceFiles !== undefined)
    validateSourceFiles(errors, value.sourceFiles);
...
function validateSource(errors: string[], value: unknown): void {
  ...
  if (type === 'bundled') {
    if (value.origin !== undefined)
      expectNonEmptyString(errors, '$.source.origin', value.origin);
    return;
  }
  ...
  if (type === 'local') {
    ...
    if (value.importedAt !== undefined)
      expectIsoDateTime(errors, '$.source.importedAt', value.importedAt);
    return;
  }
  ...
  if (type === 'github') {
    ...
    if (value.branch !== undefined)
      expectNonEmptyString(errors, '$.source.branch', value.branch);
    if (value.commit !== undefined)
      expectNonEmptyString(errors, '$.source.commit', value.commit);
    if (value.importedAt !== undefined)
      expectIsoDateTime(errors, '$.source.importedAt', value.importedAt);
    return;
  }
  ...
  if (value.registryUrl !== undefined)
    expectNonEmptyString(errors, '$.source.registryUrl', value.registryUrl);
  if (value.item !== undefined)
    expectNonEmptyString(errors, '$.source.item', value.item);
  if (value.homepage !== undefined)
    expectNonEmptyString(errors, '$.source.homepage', value.homepage);
  if (value.importedAt !== undefined)
    expectIsoDateTime(errors, '$.source.importedAt', value.importedAt);
}
...
function validateFonts(errors: string[], value: unknown): void {
  ...
  value.forEach((font, index) => {
    ...
    if (font.style !== undefined)
      expectNonEmptyString(errors, `${pathLabel}.style`, font.style);
  });
}
...
function validatePreview(errors: string[], value: unknown): void {
  ...
  value.pages.forEach((page, index) => {
    ...
    if (page.role !== undefined)
      expectNonEmptyString(errors, `${pathLabel}.role`, page.role);
    if (page.title !== undefined)
      expectNonEmptyString(errors, `${pathLabel}.title`, page.title);
  });
}
...
function validateSourceFiles(errors: string[], value: unknown): void {
  ...
  for (const key of ALLOWED_SOURCE_FILES_KEYS) {
    ...
    if (sourcePath !== undefined)
      expectSafeRelativePath(errors, `$.sourceFiles.${key}`, sourcePath);
  }
}
...
function rejectUnknownKeys(
  ...
  for (const key of Object.keys(value)) {
    if (!allowed.has(key))
      errors.push(
        `${pathLabel}.${key} is not part of the v1 design-system project schema`,
      );
  }
}
...
function expectLiteral(
  ...
  if (value !== expected)
    errors.push(`${pathLabel} must be ${JSON.stringify(expected)}`);
}
...
function expectSlugArray(
  ...
  value.forEach((entry, index) => {
    if (
      ...
      errors.push(
        `${pathLabel}[${index}] must be a lowercase slug matching /^[a-z0-9]+(?:-[a-z0-9]+)*$/`,
      );
    }
  });
}

function expectSafeRelativePath(
  errors: string[],
  pathLabel: string,
  value: unknown,
): void {
  ...
  if (
    ...
    errors.push(
      `${pathLabel} must be a safe relative path without empty, "." or ".." segments`,
    );
  }
}
...
function expectSlug(errors: string[], pathLabel: string, value: unknown): void {
  if (typeof value !== 'string' || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(value)) {
    errors.push(
      `${pathLabel} must be a lowercase slug matching /^[a-z0-9]+(?:-[a-z0-9]+)*$/`,
    );
  }
}
