...
name: live-artifact
description: |
  Create refreshable, auditable DesignMode artifacts backed by connector or local data.
...
Create an DesignMode live artifact: a project-scoped, previewable HTML artifact whose data can later be refreshed without redesigning the presentation.
...
1. **Resolve scope and data source without blocking on connected connectors**
   ...
   - If connector data is needed, first list connectors with `"$NEUMA_NODE_BIN" "$NEUMA_BIN" tools connectors list --format compact`. If the named connector is present with `status: "connected"`, choose an appropriate read-only `auto` tool from its catalog and execute it through the connector wrapper.
...
5. **Register or update through daemon wrappers**
   - Use the DesignMode daemon wrapper commands via `"$NEUMA_NODE_BIN" "$NEUMA_BIN"` instead of raw `curl`, bare `node`, or bare `od`:

     ```bash
     "$NEUMA_NODE_BIN" "$NEUMA_BIN" tools live-artifacts create --input artifact.json
     "$NEUMA_NODE_BIN" "$NEUMA_BIN" tools live-artifacts list --format compact
     "$NEUMA_NODE_BIN" "$NEUMA_BIN" tools live-artifacts update --artifact-id "$ARTIFACT_ID" --input artifact.json
     ```

   - The wrapper reads injected `NEUMA_NODE_BIN`, `NEUMA_BIN`, `NEUMA_DAEMON_URL`, and `NEUMA_TOOL_TOKEN`; do not print, persist, or override token values.
...
6. **Use connector wrappers for connector data**
   - Discover available connectors and tools:
     ...
     ```bash
     "$NEUMA_NODE_BIN" "$NEUMA_BIN" tools connectors list --format compact
     ```

   - Execute a read-only connector tool with a JSON object input file:

     ```bash
     "$NEUMA_NODE_BIN" "$NEUMA_BIN" tools connectors execute --connector "$CONNECTOR_ID" --tool "$TOOL_NAME" --input input.json
     ```
