...
- Tool endpoints require the injected `NEUMA_TOOL_TOKEN`; do not invent or pass `projectId`.
...
1. Use `notion.search_pages` with a concise query derived from the user’s requested artifact/topic.
2. If the user provided a database id or page id but the current catalog has no exact read tool for it, ask whether to search related pages instead of inventing a tool name.
...
```json
{
  ...
  "input": { "owner": "design-mode", "repo": "design-mode" },
  "connector": {
    "connectorId": "github",
    "accountLabel": "public",
    "toolName": "github.github_search_repositories"
