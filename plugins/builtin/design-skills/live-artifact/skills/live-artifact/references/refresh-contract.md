...
1. List connectors with `"$NEUMA_NODE_BIN" "$NEUMA_BIN" tools connectors list --format compact`.
...
3. Execute once with `"$NEUMA_NODE_BIN" "$NEUMA_BIN" tools connectors execute --connector <id> --tool <name> --input input.json` to produce compact normalized preview data.
