...
3. **Language** — English or another explicit user-requested language?
4. **Format** — on-screen live, PDF export, social carousel?
...
## 7. Multilingual decks

- Only use a non-English language when the user explicitly asks for it.
- Set the matching `lang` attribute on `<html>`.
- For bilingual titles, stack lines:
  `<h1 class="h1">Primary title<br><span class="dim">Secondary title</span></h1>`.
- Keep subtitles in a lighter weight and dim color to avoid visual competition.
...
for social carousel output (1242x1660).
...
- **Don't put presenter-only text on the slide.** Any descriptive text,
  narration cues, or explanations meant for the speaker (e.g. "The key point
  on this slide is...",
