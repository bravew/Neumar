---
name: html-ppt
description: HTML PPT Studio — author professional static HTML presentations in many styles, layouts, and animations, all driven by templates. Use when the user asks for a presentation, PPT, slides, keynote, deck, slideshow, reveal-style HTML deck, social carousel, or any kind of multi-slide pitch/report/sharing document that should look tasteful and support keyboard navigation. Triggers include keywords like "presentation", "ppt", "slides", "deck", "keynote", "reveal", "slideshow", "talk slides", "pitch deck", "tech sharing", and "technical presentation".
triggers:
  - "ppt"
  - "deck"
  - "slides"
...
od:
  ...
  speaker_notes: true
  animations: true
  example_prompt: "Use html-ppt to make a 12-page HTML presentation. First confirm three things: content/page count/audience, theme choice (recommend 2-3 of the 36 themes), and the starting full-deck template (pitch-deck, tech-sharing, weekly-report, xhs-post, or presenter-mode-reveal). Start writing slides only after those are aligned."
---
...
- **15 full-deck templates** (`templates/full-decks/<name>/`) — complete multi-slide decks with scoped `.tpl-<name>` CSS. 8 extracted from real-world decks (xhs-white-editorial, graphify-dark-graph, knowledge-arch-blueprint, hermes-cyber-terminal, obsidian-claude-gradient, testing-safety-alert, xhs-pastel-card, dir-key-nav-minimal), 7 scenario scaffolds (pitch-deck, product-launch, tech-sharing, weekly-report, xhs-post 3:4, course-module, and **presenter-mode-reveal** for talk delivery)
...
If the user mentions talks, presentations, speaker notes, presenter view, teleprompter-style notes, technical sharing, or a deck they need to present live, **use the `presenter-mode-reveal` full-deck template** and write 150-300 words of speaker script in each slide's `<aside class="notes">`.
...
1. **Prompt signals, not a script to read verbatim** — bold key terms and put transitions in separate paragraphs.
2. **150-300 words per slide** — a comfortable 2-3 minutes per page.
3. **Use conversational language** — write for speaking, not for a formal memo.
...
- 🟠 **SPEAKER SCRIPT** — large-font speaker notes (scrollable)
...
Only `presenter-mode-reveal` is designed from the ground up around the feature with proper example speaker notes on every slide.
...
1. **Content & audience.** What's the deck about, how many slides, who's
   watching (engineers / execs / social readers / students / VC)?
2. **Style / theme.** Which of the 36 themes fits? If unsure, recommend 2-3
   ...
   - Social carousel → `xiaohongshu-white`, `soft-pastel`, `rainbow-gradient`,
     `magazine-bold`
...
3. **Starting point.** One of the 14 full-deck templates, or scratch? Point
   ...
   user's content suggests something obvious (e.g. a product launch keynote →
   `product-launch`), propose it confidently instead of asking blindly.

A good opening message looks like:

> I can make this presentation. First, let's confirm three things:
> 1. What is the rough content, page count, and audience?
> 2. What style should it use? I recommend choosing from `tokyo-night` for technical talks, `xiaohongshu-white` for social carousel layouts, or `corporate-clean` for formal business reports.
> 3. Should I start from the existing `tech-sharing` full-deck template?
...
- **NEVER put presenter-only text on the slide itself.** Descriptive text like
  "This slide shows..." or "Speaker: add context here..." or small explanatory captions
...
into a deck, how to choose layouts and themes per audience, and how to export.
...
- [references/presenter-mode.md](references/presenter-mode.md) — presenter mode and speaker script guide.
